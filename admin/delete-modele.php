...
error_log("Données POST reçues dans delete-modele.php : " . print_r($_POST, true));

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $id = $_POST['id'] ?? null;
    $success_redirect = 'modeles.php?success=delete';
    $error_redirect = 'modeles.php?error=delete';

    // Vérification basique des données requises
    if (!$id) {
        error_log("Erreur: ID du modèle manquant pour la suppression.");
        header('Location: ' . $error_redirect . '&message=' . urlencode('ID du modèle manquant.'));
        exit;
    }
    ...
    try {
        ...
        // Supprimer les images physiques associées au modèle
        ...
        // Grâce à ON DELETE CASCADE, les entrées dans modele_images seront automatiquement supprimées.
        $stmt = $pdo->prepare("DELETE FROM modeles WHERE id = ?");
        $stmt->execute([$id]);
        error_log("Modèle ID " . $id . " supprimé.");
        ...
        error_log("Erreur PDO lors de la suppression du modèle ID " . $id . " : " . $e->getMessage());
        ...
        exit;
         ...
         error_log("Erreur générale lors de la suppression du modèle ID " . $id . " : " . $e->getMessage());
